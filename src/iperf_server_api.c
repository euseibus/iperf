...
{
    retry:
    if((test->listener = netannounce(test->settings->domain, Ptcp, test->bind_address, test->server_port, test->device_file)) < 0) {
                ...
                if (streams_accepted == test->num_streams) {
                    if (test->protocol->id != Ptcp) {
                        ...
                        if (test->no_delay || test->settings->mss || test->settings->socket_bufsize) {
                            ...
                            close(test->listener);
                            if ((s = netannounce(test->settings->domain, Ptcp, test->bind_address, test->server_port,test->device_file)) < 0) {
				cleanup_server(test);
                                i_errno = IELISTEN;
                                return -1;
                            }
